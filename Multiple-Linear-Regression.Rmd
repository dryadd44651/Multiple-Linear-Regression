...
```{r}
allFit = lm(psa~cancervol+benpros+gleason+vesinv+age+weight+capspen)
print(summary(allFit))
allFit1 = lm(logPSA~cancervol+benpros+gleason+vesinv+age+weight+capspen)
print(summary(allFit1))
#allFit2 = lm(1/psa~cancervol+benpros+gleason+vesinv+age+weight+capspen)
#print(summary(allFit2))
#allFit3 = lm(exp(psa)~cancervol+benpros+gleason+vesinv+age+weight+capspen)
...
```{r}
Fit1 = lm(logPSA~cancervol+benpros+gleason+vesinv)
print(summary(Fit1))
Fit2 = lm(logPSA~log(cancervol)+benpros+gleason+vesinv)
print(summary(Fit2))
Fit3 = lm(logPSA~log(cancervol)+log(cancervol*gleason)+benpros+gleason+vesinv)
print(summary(Fit3))
```
...
```{r}
fit <- step(lm(logPSA ~ 1), scope = list(lower = ~1, upper = ~cancervol+benpros+gleason+vesinv+age+weight+capspen), direction = "both")
print(summary(fit))
...
```{r}
library(MASS)
rfit = lm.ridge(logPSA~cancervol+benpros+gleason+vesinv+age+weight+capspen)
print(rfit$coef)
